...
minecraftserverurl=$(cat "$basedir"/work/Paper/work/BuildData/info.json | grep serverUrl | cut -d '"' -f 4)
minecraftversion=$(cat "$basedir"/work/Paper/work/BuildData/info.json | grep minecraftVersion | cut -d '"' -f 4)
decompiledir="$basedir/work/Paper/work/Minecraft/$minecraftversion"
