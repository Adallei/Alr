...
workdir="$basedir"/work/Paper/work
minecraftversion=$(cat "$basedir"/work/Paper/work/BuildData/info.json | grep minecraftVersion | cut -d '"' -f 4)
...
function import {
    if [ -f "$basedir/work/Paper/Paper-Server/src/main/java/net/minecraft/server/$1.java" ]; then
        echo "ALREADY IMPORTED $1"
        return 0
    fi
    export importedmcdev="$importedmcdev $1"
    file="${1}.java"
    target="$basedir/work/Paper/Paper-Server/src/main/java/$nms/$file"
...
for f in $files; do
    ...
    if [ "$?" == "1" ]; then
        if [ ! -f "$basedir/work/Paper/Paper-Server/src/main/java/net/minecraft/server/$f.java" ]; then
